...
public class Asteroid extends Scrollable {
    ...
    public Asteroid(float x, float y, float width, float height, float velocity, boolean haPetado) {
    ...
    public boolean peta (Disparo disparo) {
        if(!(haPetado)) {
            return (Intersector.overlaps(collisionCircle, disparo.getBalaCollision()));
        } else {
            return false;
        }
    }
